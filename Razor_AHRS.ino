...
#define EXTERNAL_TRIGGER_FRAMERATE 20 // Built in by kjh (removable constant)

#define LIDAR_TRIGGER_PIN 10 // Built in by kjh (remove if in need)
...
// Stuff
#define STATUS_LED_PIN 13  // Pin number of status LED // Repurposed as Camera trigger pin. 
...
void loop()
{
  ...
  if((millis() - timestamp) >= OUTPUT__DATA_INTERVAL) //About 7ms is used. so 5 cycle means 35ms. //Changed to 10ms per cycle.
  {
    ...
    if(rpt==0){
      //digitalWrite (EXTERNAL_TRIGGER_SIGNAL, HIGH);
      digitalWrite(STATUS_LED_PIN, HIGH);
    }
    else if(rpt==2){
      //digitalWrite (EXTERNAL_TRIGGER_SIGNAL, LOW);
      digitalWrite(STATUS_LED_PIN, LOW);
    }
